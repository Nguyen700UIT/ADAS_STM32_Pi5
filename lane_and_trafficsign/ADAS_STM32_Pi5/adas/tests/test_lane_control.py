...
_WORKSPACE_DIR = Path(__file__).resolve().parents[6]
...
# Default paths
# ---------------------------------------------------------------------------
_DEFAULT_VIDEO = os.path.join(os.path.dirname(__file__), "..", "videos", "VIDEO_GOC_TESTCASE_1.mp4")
...
def main():
    ...
    # ------------------------------------------------------------------
    # Initialise Webserver Dashboard
    # ------------------------------------------------------------------
    ...
    while True:
        ...
        update_vehicle_state(
            ...
            distance_left=last_stm32_response["distance_left"] if last_stm32_response else 999,
            distance_right=last_stm32_response["distance_right"] if last_stm32_response else 999,
            actual_rpm=last_stm32_response["actual_rpm"] if last_stm32_response else 0,
            brake_command=1 if speed == 0 else 0
        )
