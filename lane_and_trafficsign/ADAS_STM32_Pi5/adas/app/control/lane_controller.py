...
class LaneController:
    def __init__(self, port=None, baudrate=None, timeout=None, img_width=640, img_height=480, warp_matrix=None, uart=None):
        ...
        # Khởi tạo UART với cấu hình tối ưu luồng hoặc dùng chung
        if uart is not None:
            self.uart = uart
    ...
    def send_to_stm32(self, steering_val, curvature, speed, flags, is_angle=False):
        ...
        self.uart.send_data(cmd_id, target_speed, steering_error, brake_command)
        self.last_send_time = time.time()
        return True
